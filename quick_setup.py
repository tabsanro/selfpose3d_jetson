...
def run():
    for video_name, video_id in video_ids.items():
        output_path = osp.join(PWD, 'modules', 'SelfPose3d', 'data_0705', 'hdVideos', video_name)
    ...
    for calibration_name, calibration_id in calibration_ids.items():
        output_path = osp.join(PWD, 'modules', 'SelfPose3d', 'data_0705', 'calibration', calibration_name)
        if osp.exists(output_path):
    ...
    for model_name, model_id in model_ids.items():
        output_path = osp.join(PWD, 'modules', 'SelfPose3d', 'models', model_name)
        if osp.exists(output_path):
